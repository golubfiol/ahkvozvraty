...
BuildOverlay() {
    ...
    if (Profile = "Отдел возвратов") {
        ...
        yPos += 22
        Gui, Main:Add, Text, x15 y%yPos% w400, 2) Ctrl + Numpad/ - Сменить профиль
        yPos += 22
        Gui, Main:Add, Text, x15 y%yPos% w400, 3) Insert - Скрыть/Показать
        yPos += 22
        Gui, Main:Add, Text, x15 y%yPos% w400, 4) Home - Перезапуск AHK
        yPos += 22
        Gui, Main:Add, Text, x15 y%yPos% w400, 5) End - Закрыть ахк
        yPos += 22
    }
    ...
    winH := yPos + 35
    Gui, Main:Show, x0 y0 w430 h%winH% NoActivate, Overlay
...
UpdateOverlayPosition() {
    ...
    posX := ScreenW - 450
    posY := 100
    Gui, Main:Show, x%posX% y%posY% NoActivate, Overlay
}
...
; ============================================
; Ctrl + Numpad1 — ПРОФИЛЬ "Отдел возвратов": Товар с реализации
; ============================================
^Numpad1::
    if (Profile = "Дека") {
        ; === Профиль "Дека" — Создать список v-склады ===
        SetKeyDelay, 30, 20

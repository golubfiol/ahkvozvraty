...
; ============================================
; АВТОЗАПУСК (надёжная проверка через реестр)
; ============================================
AddToStartup()
...
AddToStartup() {
    ...
    if (ahkPath = "") {
        TrayTip, Автозапуск, Не найден AutoHotkey.exe. Пропиши вручную., 5, 3
        Sleep, 2000
        return
    }

    ; Формируем ожидаемое значение
    ExpectedValue := """" . ahkPath . """ """ . A_ScriptFullPath . """"

    ; Читаем текущее значение из реестра
    RegRead, CurrentValue, HKEY_CURRENT_USER\Software\Microsoft\Windows\CurrentVersion\Run, Vozvraty

    if (CurrentValue = ExpectedValue) {
        ; Уже в автозагрузке — ничего не делаем
        return
    }

    ; Прописываем (или перезаписываем, если путь изменился)
    RegWrite, REG_SZ, HKEY_CURRENT_USER\Software\Microsoft\Windows\CurrentVersion\Run, Vozvraty, %ExpectedValue%
    IniWrite, 1, %SettingsFile%, Settings, StartupAdded

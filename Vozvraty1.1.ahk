...
global UpdateUrl := "https://raw.githubusercontent.com/golufbiol/ahkvozvraty/main/latest_version.txt"
global ScriptUrl := "https://raw.githubusercontent.com/golufbiol/ahkvozvraty/main/Vozvraty.ahk"
...
global SearchNumber  := "66811"   ; для Numpad5
global SearchNumber1 := "86958"   ; для Numpad1
...
Gui, Add, Text, x15 y174 w340, Ctrl + Numpad0 - Изменить числа
...
; ============================================
; Ctrl + Numpad0 - выбор, какое число менять
; ============================================
^Numpad0::
    MsgBox, 4, Изменить число, Да - изменить "Поиск по клиентскому" (Numpad5).`nНет - изменить "Товар с реализации 1" (Numpad1).`n`n(Отмена - ничего не делать)
    IfMsgBox, Yes
    {
        InputBox, NewNum, Поиск по клиентскому, Введи число:, , 250, 130, , , , , %SearchNumber%
        if (ErrorLevel = 0 && NewNum != "") {
            SearchNumber := NewNum
            IniWrite, %SearchNumber%, %SettingsFile%, Settings, SearchNumber
            UpdateStatus("Сохранено (Numpad5): " SearchNumber)
            Sleep, 1500
            UpdateStatus("Готов")
        }
    }
    ...
    {
        InputBox, NewNum1, Товар с реализации 1, Введи число:, , 250, 130, , , , , %SearchNumber1%
        if (ErrorLevel = 0 && NewNum1 != "") {
            SearchNumber1 := NewNum1
            IniWrite, %SearchNumber1%, %SettingsFile%, Settings, SearchNumber1
            UpdateStatus("Сохранено (Numpad1): " SearchNumber1)
            Sleep, 1500
            UpdateStatus("Готов")
        }
    }

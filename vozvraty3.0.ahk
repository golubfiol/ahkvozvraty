...
; ============================================
; Проверка обновлений (UTF-8 через WinHttp)
; ============================================
CheckForUpdates()

CheckForUpdates() {
    ; === 1. Скачиваем latest_version.txt ===
    try {
    ...
    ; === 2. Скачиваем сам скрипт как UTF-8 ===
    try {
    ...
    ; BOM + содержимое в UTF-8
    FileAppend, % Chr(0xFEFF), %tempScript%, UTF-8
...
Gui, Add, Text, x15 y287 w370, 12) End - Закрыть ахк

Gui, Font, s11 cWhite Bold, Segoe UI
Gui, Add, Text, x15 y323 w370 cLime, === СТАТУС ===
Gui, Font, s10 cYellow Norm
Gui, Add, Text, x15 y348 w370 vStatusText, Готов

Gui, Show, x0 y300 w400 h389 NoActivate, Overlay
...
UpdateOverlayPosition() {
    ...
    posX := ScreenW - 420
    posY := (ScreenH - 389) // 2
    Gui, Show, x%posX% y%posY% w400 h389 NoActivate, Overlay
}
...
^NumpadAdd::
    if (CurrentTask = "1") {
        ...
        MouseClick, Left, 258, 880
        Sleep, 50
        MouseClick, Left, 259, 611
        Sleep, 50

        if (F5_UsePercent) {
            MouseClick, Left, 181, 948
            Sleep, 50
            SendInput, ^+a
            Sleep, 50
            SendInput, {Backspace}
